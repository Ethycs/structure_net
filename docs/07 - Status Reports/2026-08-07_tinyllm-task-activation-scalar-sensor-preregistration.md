# TinyLLM task-activation scalar sensor preregistration

**Status:** SUPERSEDED BEFORE OUTCOME — DO NOT EXECUTE  
**Date:** 2026-08-07  
**Conformance:** NAL-STD-EXPERIMENT `PREREGISTERED`, `UNDERPOWERED`, frozen-checkpoint post-outcome mechanistic diagnostic  
**Hypothesis:** `tinyllm-c2-task-activation-scalar-sensor-v1`  
**Schema:** `nal.tinyllm-c2-task-activation-scalar-sensor.v1`

> Supersession note: this protocol and the observable scalar-residual protocol
> reserved the same fresh-D seeds (`530007/530008`). No outcome from this
> activation-heavy protocol was generated or inspected. The narrower
> `tinyllm-c2-observable-scalar-residual-v1` protocol is authoritative because
> its primary arm tests the frozen continuation posterior before escalating to
> local activation context. Reusing cohort D here would double-spend the fresh
> evidence, so this runner is intentionally execution-locked.
...
summaries are fitted on source cohorts A/B only. Fresh-D outcomes have not been
...
Use a new fresh-D cohort with seeds:

```text
composition   530007
extrapolation 530008
```
...
PCA rank, feature arm, or checkpoint is selected using fresh-D outcomes.
...
For every arm report source and fresh-D:
...
when, in each checkpoint after pooling both fresh-D shifts:
...
1. `zero` fails and `exact`, `direct_rank3`, `local_oracle`, and
   `source_covector_oracle_error` pass both fresh-D cells;
...
- runner:
  `experiments/structure_net/tinyllm_task_activation_scalar_sensor.py`
- tests:
  `tests/structure_net/test_tinyllm_task_activation_scalar_sensor.py`
- systems-only root:
  `data/experiments/tinyllm_task_activation_scalar_sensor/20260807_shakedown_cuda`
- primary root:
  `data/experiments/tinyllm_task_activation_scalar_sensor/20260807_d6_fresh_d`
- report:
  `docs/08 - Analysis/2026-08-07_tinyllm-task-activation-scalar-sensor.md`
- meta hypothesis:
  `tinyllm-c2-task-activation-scalar-sensor-v1`

Focused CPU contracts, a real CUDA lifecycle, strict JSON, implementation
