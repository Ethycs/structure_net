#!/usr/bin/env python3
"""Store the TinyLLM deck-irrep fusion-ablation result in the NAL ledger."""
...
def main() -> None:
    ...
    parser.add_argument("--results", type=Path, default=Path("data/experiments/tinyllm_irrep_fusion_ablation/20260806_d6_preregistered/campaign_results.json"))
    parser.add_argument("--output", type=Path, default=Path("data/meta_hypotheses/tinyllm-deck-irrep-fusion-ablation-v1.json"))
    ...
    record = store_irrep_fusion_ablation_meta_hypothesis(
        args.results,
        args.output,
