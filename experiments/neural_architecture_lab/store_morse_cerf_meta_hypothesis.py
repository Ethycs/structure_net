#!/usr/bin/env python3
"""Store the TinyLLM Morse--Cerf result in the NAL ledger."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--results",
        type=Path,
        default=Path("data/experiments/tinyllm_morse_cerf/20260806_d6_preregistered/campaign_results.json"),
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("data/meta_hypotheses/tinyllm-equivariant-morse-cerf-quotient-front-v1.json"),
    )
    ...
    record = store_morse_cerf_meta_hypothesis(
        args.results,
        args.output,
