...
best_performers = df.loc[df.groupby('Category')['Accuracy'].idxmax()]

# Create plot
plt.style.use('seaborn-v0_8-whitegrid')
fig, ax = plt.subplots(figsize=(12, 8))

sns.barplot(data=best_performers, x='Category', y='Accuracy', hue='Dataset', ax=ax, dodge=False)

ax.set_title('Best Performing Model by Experiment Category', fontsize=16)
ax.set_xlabel('Experiment Category', fontsize=12)
ax.set_ylabel('Best Test Accuracy', fontsize=12)
ax.set_xticklabels(ax.get_xticklabels(), rotation=45, ha='right')
plt.tight_layout()
plt.savefig('best_by_category.png')
print('Plot saved to best_by_category.png')
