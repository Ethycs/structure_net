...
from experiments.structure_net import tinyllm_task_activation_scalar_sensor as sensor
...
def test_protocol_is_superseded_and_execution_locked(tmp_path: Path) -> None:
    assert sensor.SUPERSEDED_BY == "tinyllm-c2-observable-scalar-residual-v1"
    with pytest.raises(RuntimeError, match="superseded before outcome"):
        sensor.run_campaign(_config(), tmp_path)


def test_primary_configuration_and_fresh_d_are_locked() -> None:
